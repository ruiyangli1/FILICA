...
data_generate_MAR = function(nsubj = 100, nvoxel1 = 1000, nvoxel2 = 3000, true_ncomp = 2, seed = 1234, n_miss = 5, es_C1H1 = 0.5, es_C2H2 = 0.5){
  # Y(k) = XW(k) H + E(k)

  set.seed(seed)

  # covariate: nsubjx1, for H and missing subjects
  ## C1: a continuous variable, e.g., ~N(0,1)
  ## C2: a categorical variable, e.g., binary for now
  # H: ncomp x nsubj, comp1 correlated w/ C1, comp2 correlated w/ C2
  C1H1 = mvtnorm::rmvnorm(n = nsubj, mean = rep(0, 2), sigma = matrix(c(1,es_C1H1,es_C1H1,1), nrow = 2, ncol = 2)) # sigma: covariance matrix
  #dim(C1H1); cor(C1H1)
  C1 = C1H1[,1]
  H1 = C1H1[,2]
  #cor(C1,H1)
  C2 = rbinom(n = nsubj, size = 1, prob = 0.5) #or C2 = sample(c(0,1), size = nsubj, replace = TRUE, prob = c(0.5,0.5)); #or 3-category [needs to be transferred to dummy variables]: C2 = sample(c("A","B","c"), size = nsubj, replace = TRUE, prob = c(0.5, 0.2, 0.3))
  H2 = es_C2H2 * C2 + rnorm(nsubj, 0, 1)
  #cor(H2, C2)
